...
class Help(commands.Cog, name='Help'):
    ...
    async def game_stats(self, ctx):
        embed = functions.my_embed('Game Stats', 'Returns game stats for the specified player for the specified year and date. Only available for Patrons. Must run add-league command first.', discord.Colour.blue(), '**Syntax**', '<prefix>game-stats [first name] [last name] [team abbreviation] [year] [date in mm/dd format, if month or day is single number such as 9, use 9 not 09]', False, ctx)
        await ctx.send(embed=embed)
