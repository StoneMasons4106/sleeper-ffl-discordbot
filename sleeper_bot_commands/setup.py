...
def set_channel(message, bot, channel_id: str):
    if message.guild == None:
        ...
        if message.author.guild_permissions.administrator:
                    ...
                    if isinstance(channel, discord.channel.TextChannel):
                        if str(channel.id) == str(channel_id):
                            ...
                            if existing_channel:
                                ...
                                MONGO_CONN.close()
                                embed = functions.my_embed('Channel Connection Status', 'Result of Channel Connection request', discord.Colour.blue(), 'Channel', 'Successfully updated your channel to '+str(channel_id)+'!', False, bot)
                                break
                            else:
                            ...
                            if found == 0:
                                embed = functions.my_embed('Channel Connection Status', 'Result of Channel Connection request', discord.Colour.blue(), 'Channel', 'Invalid channel ID. Try right clicking the Discord channel and hitting Copy ID.', False, bot)
                            else:
                                pass
                        else:
                            continue
            ...
            embed = functions.my_embed('Channel Connection Status', 'Result of Channel Connection request', discord.Colour.blue(), 'Channel', 'You do not have access to this command, request failed.', False, bot)
    return embed
        ...
        if ctx.author.guild_permissions.administrator:
            if league_id.isnumeric():
                ...
                if hasattr(league, 'response'):
                    embed = functions.my_embed('Sleeper League Connection Status', 'Result of connection to Sleeper League request', discord.Colour.blue(), 'Connection Status', 'Invalid league ID. Try finding the ID using your league URL like so: https://sleeper.app/leagues/league_id', False, bot) 
                else:
            ...
            embed = functions.my_embed('Sleeper League Connection Status', 'Result of connection to Sleeper League request', discord.Colour.blue(), 'Connection Status', 'You do not have access to this command, request failed.', False, bot)
    return embed
        ...
        if message.author.guild_permissions.administrator:
            if score_type == 'pts_ppr' or score_type == 'pts_half_ppr' or score_type == 'pts_std': 
                ...
                if existing_league:
                    ...
                    MONGO.servers.update_one(existing_league, newvalue)
                    MONGO_CONN.close()
                    embed = functions.my_embed('Sleeper League Score Type', 'Result of attempt to update score type for your League', discord.Colour.blue(), 'Score Type Request Status', f'Successfully updated your score type to {score_type_output}!', False, bot)
                else:
                    if score_type == 'pts_ppr':
                        score_type_output = 'PPR'
                    elif score_type == 'pts_half_ppr':
            ...
            embed = functions.my_embed('Sleeper League Score Type', 'Result of attempt to update score type for your League', discord.Colour.blue(), 'Score Type Request Status', 'You do not have access to this command.', False, bot)
    return embed