...
def forecast(bot, city: str):
    ...
    if forecast.status_code == 200:
        ...
        embed = functions.my_embed('Weather Forecast', f'3 day forecast for {forecast.json()["location"]["name"]}, {forecast.json()["location"]["region"]}', discord.Colour.blue(), f'Forecast for {city}', forecast_string, False, bot)
    else:
...
def current_weather(bot, city: str):
    ...
    if current_weather.status_code == 200:
        ...
        embed = functions.my_embed('Current Weather', f'Current weather for {current_weather.json()["location"]["name"]}, {current_weather.json()["location"]["region"]}', discord.Colour.blue(), f'Current Weather for {city}', current_weather_string, False, bot)
    else:
        embed = 'Invalid city name or zip code, please try again!'
    return embed