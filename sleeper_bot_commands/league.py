        ...
        if existing_league:
            if "league" in existing_league:
                ...
                embed = functions.my_embed(
                    "Sleeper League Info",
                    "Sleeper League General Information",
                    discord.Colour.blue(),
                    "Name",
                    f'[{league["name"]}](https://sleeper.app/leagues/{league_id})',
                    False,
                    bot,
                )
                ...
                embed.add_field(
                    name="Trade Deadline",
                    value=f"Week {league['settings']['trade_deadline']}",
                    inline=False,
                )
                embed.add_field(
                    name="Playoffs Start",
                    value=f"Week {league['settings']['playoff_week_start']}",
                    inline=False,
                )
                if "score_type" in existing_league:
                    if existing_league["score_type"] == "pts_ppr":
                        ...
                        embed.add_field(
                            name="Scoring Type", value="Half PPR", inline=False
                        )
                    elif existing_league["score_type"] == "pts_std":
                        embed.add_field(
                            name="Scoring Type", value="Standard", inline=False
                        )
                else:
                    pass
            else:
                embed = functions.my_embed(
                    "Sleeper League Info",
                    "Sleeper League Name and Member Info",
                    discord.Colour.blue(),
                    "Members",
                    "No league specified, run add-league command to complete setup.",
                    False,
                    bot,
                )
        else:
            ...
            if "league" in existing_league:
                ...
                for roster in rosters_object:
                    if roster["owner_id"] != None:
                        ...
                        pass
                standings_object = sleeper_wrapper.League(int(league_id)).get_standings(
                    filtered_roster_object, users_object
                )
                standings_string = ""
                count = 0
                ...
                embed = functions.my_embed(
                    "Sleeper League Standings",
                    "Display Current Standings of Sleeper League",
                    discord.Colour.blue(),
                    "Standings",
                    standings_string,
                    False,
    ...
    if ctx.guild == None:
        ...
        if week.isnumeric():
            if int(week) <= 18 and int(week) >= 1:
                ...
                embed = (
                    "Invalid week number given. Choose a valid week between 1 and 18."
                )
        else:
                ...
                if existing_league:
                    if "league" in existing_league:
                        ...
                        if matchups:
                            ...
                            embed = functions.my_embed(
                                f"Week {week} Scoreboard",
                                f"Scoreboard for Week {str(week)}",
                                discord.Colour.blue(),
                                "Scoreboard",
                                scoreboard_string,
                                False,
