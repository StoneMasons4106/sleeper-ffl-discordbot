    ...
    if add_drop == 'add' or add_drop == 'drop':
        ...
        for player in trending_players:
            ...
            if add_drop == 'add':
                ...
                trending_string += f'{str(count)}. {db_player["name"]} {db_player["position"]} - {team} -{str(change)}\n'
        if add_drop == 'add':
            embed = functions.my_embed('Trending Players', 'Display Current Trending Added Players', discord.Colour.blue(), 'Players', trending_string, False, bot)
        else:   
            embed = functions.my_embed('Trending Players', 'Display Current Trending Dropped Players', discord.Colour.blue(), 'Players', trending_string, False, bot)
    else:
                ...
                if "league" in existing_league:
                    ...
                    if user:
                        ...
                        if roster_portion == 'starters':
                            ...
                            MONGO_CONN.close()
                            embed = functions.my_embed('Roster', f'Starting Roster for {user["display_name"]}', discord.Colour.blue(), 'Starting Roster', starters_string, False, bot)
                        if roster_portion == 'all':
                            ...
                            try:
                                if len(users_roster["players"]) != 0:
                                    ...
                                    MONGO_CONN.close()
                                    embed = functions.my_embed('Roster', f'Complete Roster for {user["display_name"]}', discord.Colour.blue(), 'Full Roster', players_string, False, bot)
                                else:
                                    players_string = 'None'
                                    embed = functions.my_embed('Roster', f'Complete Roster for {user["display_name"]}', discord.Colour.blue(), 'Full Roster', players_string, False, bot)
                            except:
                        ...
                        if roster_portion == 'bench':
                            ...
                            try:
                                if len(users_roster["players"]) != 0:
                                    ...
                                    MONGO_CONN.close()
                                    embed = functions.my_embed('Roster', f'Bench for {user["display_name"]}', discord.Colour.blue(), 'Bench', bench_string, False, bot)
                                else:
                                    bench_string = 'None'
                                    embed = functions.my_embed('Roster', f'Bench for {user["display_name"]}', discord.Colour.blue(), 'Bench', bench_string, False, bot)
                            except:
...
def status(bot, first_name, last_name, team_abbreviation):
    ...
    if existing_player:
        embed = functions.my_embed('Status', f'Roster, Injury, and Depth Chart Status for {first_name} {last_name} - {team_abbreviation}', discord.Colour.blue(), 'Roster Status', existing_player["status"], False, bot)
                        ...
                        if roster["players"] != None:
                                ...
                                if player == existing_player["id"]:
                                    for user in users:
                                        if user["user_id"] == roster["owner_id"]:
                                            embed = functions.my_embed('Who Has...', f'Command to find who currently has a particular player on their roster.', discord.Colour.blue(), f'Owner of {first_name} {last_name}', user["display_name"], False, bot)
                                            found = True
                                            break
                                        else:
                                            pass
                            ...
                            if count == len(rosters):
                                if found == True:
                                    ...
                                    embed = functions.my_embed('Who Has...', f'Command to find who currently has a particular player on their roster.', discord.Colour.blue(), f'Owner of {first_name} {last_name}', 'None', False, bot)
                                    break
                            else:
                                continue
                        else:
